...
class DailySchedule:
    ...
    Re-timed 2026-07-16 (was 9:20/1:30/3:50). The old 9:20 slot ran
    pre-open on purpose ("queue orders for the opening cross"), but in
    practice that meant risk math computed on thin pre-market IEX quotes
    and fills taken in the opening auction -- the widest-spread window of
    the day. The 10:00 slot instead trades post-open against real, settled
    quotes; 12:45 catches midday news; 3:30 sees the full day with room
    for fills before the closing auction. Must stay in sync with
    ...
    slot_times: tuple[dtime, ...] = (dtime(10, 0), dtime(12, 45), dtime(15, 30))
...
def compute_lookback_start(
    ...
    Behavior:
      - If `now` is at or before the first scheduled slot of the day (or
        earlier -- e.g. a manual/ad-hoc run before 10:00am), looks back to
      ...
      - Otherwise, looks back to the immediately preceding scheduled slot
        the same day (e.g. the 12:45pm run looks back to 10:00am).
    """
